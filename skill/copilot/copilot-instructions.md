# archiver-rag — GitHub Copilot instructions

> **Install:** Copy this file to `.github/copilot-instructions.md` in your repo
> (or merge it into an existing one). GitHub Copilot loads this file into every
> chat in the repo automatically — these rules are always active, there is no
> on-demand skill to invoke. For path-scoped rules instead, use
> `.github/instructions/archiver-rag.instructions.md` with `applyTo: "**"`
> frontmatter.

## MCP registration

Add the archiver-rag server to `.vscode/mcp.json` (VS Code) so Copilot Chat can
call its tools in agent mode:
